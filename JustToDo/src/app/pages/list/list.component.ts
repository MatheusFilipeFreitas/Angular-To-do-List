...
export class ListComponent implements OnInit {
  ...
  add(){
    ...
    this.itemService.postItem(this.item).subscribe(
      {
        next: (data: Item) => {console.log("Item cadastrado com sucesso >> ", data)}
      }
    )
    location.reload();
  }
  ...
  setStatus(item: Item){
    ...
    this.itemService.putItem(item, item.id).subscribe(
      {
        next: (data: Item) => {console.log("Item alterado com sucesso >> ", data)}
      })
  }

  delete(item: Item){
    this.itemService.deleteItem(item.id).subscribe({
      next: (data: Item) => {console.log("Item deletado com sucesso >> ", data)}
    })
    location.reload();
  }
